...
public class DebtExportService {
    ...
    // --- Colors (Debt/Liability Theme) ---
    private static final Color COL_PRIMARY_BG = new Color(128, 0, 0);     // Deep Maroon
    ...
    private static final Font FONT_CARD_AMOUNT = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 15, new Color(255, 204, 204)); // Light Red/Pink for contrast
    ...
    private void addReportTitle(Document document) throws DocumentException {
        Paragraph p = new Paragraph("Liability & Debt Statement", FONT_TITLE);
        p.setAlignment(Element.ALIGN_CENTER);
        p.setSpacingAfter(30);
        document.add(p);
    }

    private void addDebtCard(Document document, DebtResponseDTO debt) throws DocumentException {
        ...
        // 1. Header Section (Name + Outstanding Amount)
        ...
        // Name & Creditor
        String titleText = value(debt.getName()) + " (" + value(debt.getCreditor()) + ")";
        ...
        // Outstanding Amount (Key Metric for Debt)
        String amountText = "Due: " + formatAmount(debt.getOutstandingAmount());
        PdfPCell priceCell = new PdfPCell(new Phrase(amountText, FONT_CARD_AMOUNT));
        ...
        // Core Details (Category, Original Amount, Due Date)
        bodyCell.addElement(createCoreDetailsTable(debt));

        // Separator Line
        bodyCell.addElement(createSeparator());

        // Dynamic Attributes
        bodyCell.addElement(createAttributesGrid(debt.getAttributes()));
    ...
    private PdfPTable createCoreDetailsTable(DebtResponseDTO debt) {
        PdfPTable table = new PdfPTable(4);
        table.setWidthPercentage(100);
        table.setSpacingAfter(5);

        // Row 1: Category | Total Original Amount
        addKvCell(table, "Category", value(debt.getMainCategory()) + " - " + value(debt.getCategory()));
        addKvCell(table, "Original Principal", formatAmount(debt.getTotalAmount()));

        // Row 2: Creditor | Due Date
        addKvCell(table, "Lender / Creditor", value(debt.getCreditor()));
        addKvCell(table, "Next Due Date", formatDate(debt));
    ...
    private PdfPTable createAttributesGrid(Map<String, Object> attributes) {
        ...
        // Section Header
        PdfPCell titleCell = new PdfPCell(new Phrase("Loan Details & Terms", FONT_SECTION_HEADER));
    ...
    private String formatAttributeValue(String key, Object value) {
        ...
        if (k.contains("rate") || k.contains("interest")) return value + "%";
        if (k.contains("tenure") || k.contains("term")) return value + " Months";
    ...
    private String formatDate(DebtResponseDTO d) {
        return d.getDueDate() == null ? "-" : d.getDueDate().format(DATE_FORMAT);
    }
